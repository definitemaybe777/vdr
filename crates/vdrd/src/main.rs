...
use vdr_core::{process_core_dump, resolve_exe_path, CrashMetadata, StorageConfig};
...
fn verify_perms(path: &str, expected: u32, kind: &str) -> Result<()> {
    ...
    .with_context(|| format!("failed to stat {} {}", kind, path))?;
    ...
    if mode != expected || uid != 0 || gid != 0 {
        bail!(
            ...
            kind, path, mode, uid, gid, expected
        );
    }
    Ok(())
}
